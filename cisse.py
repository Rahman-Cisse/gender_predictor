...
def predict_gender(event=None):
    def fetch_data():
        try:
            ...
            gender_image.config(image=male_image if gender == 'male' else female_image)

            date_today = datetime.now().strftime('%Y-%m-%d')
            session_history.append((name, gender, probability, date_today))
            save_history()
...
def save_history():
    try:
        with open('history.json', 'a') as file:
            for name, gender, probability, date_today in session_history:
            ...
            try:
                with open('history.json', 'r') as file:
                    for line in file:
                        ...
                        listboxa.insert(0, f"Name: {prediction['name']}, Gender: {prediction['gender']}, Accuracy: {prediction['probability']:.2f}%, Date: {prediction['date']}")
